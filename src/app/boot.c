...
bool xmodem_128B_rec_bin() {
    boot_t* bp = &boot0;
    if (bp->flag & IAP_XMODEM_FLAG) {
        uart_t* up = &u0;
        // 发送 'C' 握手
        if (bp->time_out >= BOOT_TIME_OUT) {
            bp->time_out = 0;
            printf("C");
        }
        // 数据帧
        if (up->rx_size >= 133 && up->rx_buf[0] == 0x01) {
            uint16_t calc = crc16(&up->rx_buf[3], 128);
            uint16_t recv = (up->rx_buf[131] << 8) | up->rx_buf[132];
            if (calc == recv) {
                // CRC成功 → 保存到boot0->buf
                uint16_t offset = (bp->modem_num % 8) * 128; // 8块=1KB
                memcpy(&bp->buf[offset], &up->rx_buf[3], 128);
                bp->modem_num++;
                if ((bp->modem_num % 8) == 0) {
        ...
        // 结束帧
        if (up->rx_size == 1 && up->rx_buf[0] == 0x04) {
...
bool xmodem_1k_rec_bin() {
    ...
    // STX包 (1024字节)
    if (up->rx_size >= 1029 && up->rx_buf[0] == 0x02) {
        if ((up->rx_buf[1] + up->rx_buf[2]) != 0xFF) {
            printf("%c", NAK);
            up->rx_size = 0;
            return 0;
        }

        uint16_t calc = crc16(&up->rx_buf[3], 1024);
        uint16_t recv = (up->rx_buf[1027] << 8) | up->rx_buf[1028];

        if (calc == recv) {
            memcpy(bp->buf, &up->rx_buf[3], 1024);
            download_new_app(A_START_ADDR, 8);
            bp->modem_num++;
            printf("%c", ACK);
        } else {
    ...
    // EOT包
    if (up->rx_size == 1 && up->rx_buf[0] == EOT) {
        printf("%c", ACK);
        up->rx_size = 0;
        return 1;
    }
    ...
    // CAN包
    if (up->rx_size == 1 && up->rx_buf[0] == 0x18) {
        printf("%c", CAN);
        up->rx_size = 0;
        return 0;
    }
